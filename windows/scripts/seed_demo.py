...
def add(slug, name, url, *, enabled=True, status="ok", detail="", tools=0):
    store.add_server(slug, name, url, "none", None, "http", "", "[]", "{}")
...
def main():
    ...
    # 健康 —— 綠色條,第二行是等寬的網址
    add("orgpulse", "orgPulse", "https://mcp.example.com/mcp", tools=16)

    # 停用 —— 不給顏色只是變淡。它不是壞掉,不該和異常同一個視覺層級
    add("huanlan", "huanlan", "https://admin.example.org/mcp",
        enabled=False, tools=29)

    # 連不上 —— 紅色條,而且第二行從等寬的網址換成系統字的錯誤原因
    add("ap", "ap", "http://localhost:8081/mcp",
        status="error", detail="All connection attempts failed", tools=16)

    # 待確認 —— 側邊欄的計數會著琥珀色
    for tool in ("orgpulse__get_issue_by_id", "confirm_weather"):
